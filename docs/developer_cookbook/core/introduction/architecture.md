...
`实体管理` 对 core 中的实体进行管理，并代理实体相关的 [APIs](../../../api/Core/tag) 操作，为实体和实体运行时（[Actor](../specs/actor)）向外暴露接口。
...
[Storege](../specs/actor) 是 core 持久化缓冲模块，用于封装  core 用到的核心中间件资源，并使用缓冲区对时序数据的读写进行优化。
